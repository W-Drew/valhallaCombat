...
{
public:
	enum class spiritLevel
	{
		gray = 0, //-30% damage
		white = 1, //20% damage increase, 1 heavy
		yellow = 2, //40% damage increase, 2 heavy
		red = 3 //
	};

	static spiritHandler* GetSingleton()
	{
		static spiritHandler singleton;
...
private:
	void modSpirit(float a_in);


	float _currentSpirit;
